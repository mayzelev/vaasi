...
export const validationSchema = Yup.object({
    username: Yup.string()
        .matches(/^[a-zA-Zа-яА-ЯёЁіІїЇєЄґҐ'.\-\s]{1,62}$/, 'ПІБ повинно містити тільки літери, пробіли, апострофи, крапки та дефіси.')
        .required("Обов'язкове поле"),
    phone: Yup.string()
        .matches(/^\+380\d{9}$/, 'Невірний формат телефону. Використовуйте формат +380XXXXXXXXX.')
        .required("Обов'язкове поле"),
        ...
        authFunction(data)
            ...
            .catch((e) => {
                ...
                if (e?.response?.data?.message.includes(USER_EMAIL_EXIST)) {
                    errors.email = 'Такий E-mail вже зареєстровано!';
                }
