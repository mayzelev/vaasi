...
export default function BalanceCodePage() {
    ...
    const { title } = mockDataBalanceCode;
    ...
    return (
        <section className={`container ${style.balanceCode}`}>
            <div>
                <div className={style.container}>
                    <div className={style.tooltipWrapper}>
                        <h1 className="titleWithBorder">{title}</h1>
                        <TooltipComponent
                            title="Баланс кода VAASI - це баланс, який відображається після перевірки вашого коду. Отримати код ви можете після стирання захисного покриття на вашій скретч картці. Але будьте уважні, стирати покриття можна тільки після оплати заявки згідно з надісланим вам рахунком."
                            tooltipStyles={tooltipStyles}
                        />
                    </div>
                    <p className="description"></p>
                    <div>
                        <label className={style.label}>Введіть код балансу VAASI</label>
                        <TextField
                            autoComplete="true"
                            fullWidth
                            variant="outlined"
